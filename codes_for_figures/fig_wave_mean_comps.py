...
data_dir = '/data1/roms_dd_waves/analysis_outs/NIW/EP_flux_decomposed/'
...
ds_maps = xr.open_mfdataset(data_dir + 'production_terms_slice_*.nc', parallel=True)
dates = ds_maps.ocean_time.to_index()

###--> areas
lon_rho = ds_maps.lon_rho.compute()
lat_rho = ds_maps.lat_rho.compute()
dzs = '/data1/roms_dd_waves/analysis_outs/dz/niws/'
z = xr.open_mfdataset(dzs + f'dz_*.nc').z_rho

##---> Normal
zcross_A = z.isel(eta_rho=A[1],xi_rho=slice(1,-1),s_rho=slice(0,-1)).values #match vert_A


mask_normal = create_date_mask(dates, date_ranges_normal)
hor_normal = ds_maps.isel(ocean_time=mask_normal).P_hor_int.mean(dim='ocean_time').compute()
vert_normal = ds_maps.isel(ocean_time=mask_normal).P_vert_int.mean(dim='ocean_time').compute()
vert_A = xr.open_dataset(data_dir + 'production_terms_slice_18.nc').P_vert_section.sel(eta_rho=A[1]).isel(ocean_time=mask_normal).mean(dim='ocean_time').compute()



###########------cf
zcross_B = z.isel(eta_rho=B[1],xi_rho=slice(1,-1),s_rho=slice(0,-1)).values 

mask_cf = create_date_mask(dates, date_ranges_cf)
hor_cf = ds_maps.isel(ocean_time=mask_cf).P_hor_int.mean(dim='ocean_time').compute()
vert_cf = ds_maps.isel(ocean_time=mask_cf).P_vert_int.mean(dim='ocean_time').compute()
vert_B = xr.open_dataset(data_dir + 'production_terms_slice_14.nc').P_vert_section.sel(eta_rho=B[1]).isel(ocean_time=mask_cf).mean(dim='ocean_time').compute()


#######-----#hurricane
zcross_D = z.isel(eta_rho=D[1],xi_rho=slice(1,-1),s_rho=slice(0,-1)).values 

mask_hurricane = create_date_mask(dates, date_ranges_hurricane)
hor_hurricane = ds_maps.isel(ocean_time=mask_hurricane).P_hor_int.mean(dim='ocean_time').compute()
vert_hurricane = ds_maps.isel(ocean_time=mask_hurricane).P_vert_int.mean(dim='ocean_time').compute()
vert_D = xr.open_dataset(data_dir + 'production_terms_slice_3.nc').P_vert_section.sel(eta_rho=D[1]).isel(ocean_time=mask_hurricane).mean(dim='ocean_time').compute()


h = xr.open_mfdataset(dzs + f'dz_*.nc').isel(s_rho = 0).z_rho*-1
...
gs.update(left=0.1, right=0.93, wspace=0.1, hspace=0.1, top=0.96, bottom=0.1)
...
ax1.text(0.03, 0.08, '(b)', transform=ax1.transAxes, fontsize=10, fontweight='bold', va='top', ha='left')
# Define colormap and normalization

ax1.set_ylim(bottom=-32.6, top=-20)
ax1.set_xlim(left=-52, right=-39.2)
# Contourf with normalization
...
ax2.text(0.03, 0.08, '(c)', transform=ax2.transAxes, fontsize=10, fontweight='bold', va='top', ha='left')
# Define colormap and normalization
...
ax.text(0.03, 0.08, '(d)', transform=ax.transAxes, fontsize=10, fontweight='bold', va='top', ha='left')
# Define colormap and normalization
...
ax1.text(0.03, 0.08, '(e)', transform=ax1.transAxes, fontsize=10, fontweight='bold', va='top', ha='left')
# Define colormap and normalization
...
ax2.text(0.03, 0.08, '(f)', transform=ax2.transAxes, fontsize=10, fontweight='bold', va='top', ha='left')
# Define colormap and normalization
...
ax2.scatter(lon_rho[A[1],:], lat_rho[A[1],:],c=colors_c[0],s=0.05,marker='.')
ax2.scatter(lon_rho[B[1],:], lat_rho[B[1],:],c=colors_c[1],s=0.05,marker='.')
ax2.scatter(lon_rho[C[1],:], lat_rho[C[1],:],c=colors_c[2],s=0.05,marker='.')
ax2.scatter(lon_rho[D[1],:], lat_rho[D[1],:],c=colors_c[3],s=0.05,marker='.')
